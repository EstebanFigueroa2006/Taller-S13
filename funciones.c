...
void buscarPorCriterios(Sistema *sistema) {
    ...
    printf("2. Buscar por rango de precio\n");
    printf("3. Buscar por rango de anio\n");
    printf("4. Buscar por tipo (Nuevo/Usado)\n");
    printf("5. Buscar solo disponibles\n");
    ...
    int anioMin, anioMax, tipoBuscar, categoriaBuscar;
    ...
    switch (opcion) {
        ...
        case 2:
            precioMin = validarDouble("Precio minimo: ");
            precioMax = validarDouble("Precio maximo: ");
            
            for (int i = 0; i < sistema->numVehiculos; i++) {
                if (sistema->vehiculos[i].precio >= precioMin && 
                    sistema->vehiculos[i].precio <= precioMax) {
                    mostrarVehiculo(sistema->vehiculos[i]);
                    encontrados++;
                }
            }
        ...
        case 3:
            anioMin = validarEntero("Anio minimo: ");
            anioMax = validarEntero("Anio maximo: ");
            
            for (int i = 0; i < sistema->numVehiculos; i++) {
                if (sistema->vehiculos[i].anio >= anioMin && 
                    sistema->vehiculos[i].anio <= anioMax) {
                    mostrarVehiculo(sistema->vehiculos[i]);
                    encontrados++;
                }
            }
        ...
        case 4:
            tipoBuscar = validarEntero("Tipo (0=Nuevo, 1=Usado): ");
            
            if (tipoBuscar >= 0 && tipoBuscar <= 1) {
                for (int i = 0; i < sistema->numVehiculos; i++) {
        ...
        case 6:
            ...
            categoriaBuscar = validarEntero("Seleccione la categoria a buscar (0-7): ");
            
            if (categoriaBuscar >= 0 && categoriaBuscar <= 7) {
                for (int i = 0; i < sistema->numVehiculos; i++) {
                    if (sistema->vehiculos[i].tipoCarro == categoriaBuscar) {
                        mostrarVehiculo(sistema->vehiculos[i]);
                        encontrados++;
                    }
                }
...
void modificarCliente(Sistema *sistema) {
    ...
    printf("4. Modificar presupuesto\n");
    printf("0. Cancelar\n");
